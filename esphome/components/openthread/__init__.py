...
from .const import (
    CONF_DEVICE_TYPE,
    CONF_ENABLE_WAKEUP_COORDINATION,
    CONF_EXT_PAN_ID,
    CONF_FORCE_DATASET,
    ...
    cv.Schema(
        {
            ...
            cv.Optional(CONF_ENABLE_WAKEUP_COORDINATION): cv.boolean,
        }
...
def _final_validate(config):
    ...
    # Call here to have access to full_config
    _build_additional_config_file(config, full_config)
...
def _build_additional_config_file(config, full_config):
    """Called before code generation starts."""
    # how to destinguish by device, so that parallel compiles can happen?
    header_path = os.path.join(COMPONENT_DIR, "openthread_additional_config.h")

    with open(header_path, "w", encoding="utf-8") as f:
        f.write("// Auto-generated by ESPHome OpenThread component — do not edit\n")
        f.write("#pragma once\n\n")
        ...
        if config.get(CONF_ENABLE_WAKEUP_COORDINATION):
            device_type = config.get(CONF_DEVICE_TYPE)
            if device_type == "FTD":
                f.write("#define OPENTHREAD_CONFIG_WAKEUP_COORDINATOR_ENABLE 1\n")
            if device_type == "MTD":
                f.write("#define OPENTHREAD_CONFIG_WAKEUP_END_DEVICE_ENABLE 1\n")


@coroutine_with_priority(CoroPriority.COMMUNICATION)
async def to_code(config):
    add_idf_sdkconfig_option("CONFIG_OPENTHREAD_HEADER_CUSTOM", True)
    add_idf_sdkconfig_option(
        "CONFIG_OPENTHREAD_CUSTOM_HEADER_PATH", "src/esphome/components/openthread"
    )
    add_idf_sdkconfig_option(
        "CONFIG_OPENTHREAD_CUSTOM_HEADER_FILE_NAME", "openthread_additional_config.h"
    )
